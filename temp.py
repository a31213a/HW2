...
def epsilon_soft_update(pi: np.ndarray,q: np.ndarray) -> np.ndarray:# epsilon-greedy exploration: update policy matrix
...
def epsilon_soft_select_act(pi_s: np.ndarray) -> int:  # epsilon-greedy exploration: select act with specify state
...
parser.add_argument("--limit", help="input int >=3, upper limit and -1*lower limit",type=int)
parser.add_argument("--gamma", help="input 0~1, discount factor",type=float)
parser.add_argument("--epsilon", help="input 0~1, probability of discover",type=float)
parser.add_argument("--epsilon_discount", help="input 0~1, discount of epsilon",type=float)
parser.add_argument("--discover_percent", help="input 0~1, percent of episode use to discover",type=float)
parser.add_argument("--discover_epsilon", help="input 0~1, minimun epsilon in discover stage",type=float)
parser.add_argument("--episode",help="input int, need to be larger if the limit is larger",type=int)
...
returns=np.zeros((2,episode, (upper_limit-lower_limit)**2,9)) #cumulative discounted Return g-table ;2(t g)* episode * state * action, -99999->方便辨識
q=np.random.rand((upper_limit-lower_limit)**2,9) #q-table
pi=np.full(((upper_limit-lower_limit)**2,9),0.111)# creat initial policy matrix (state-act chance table)
...
for i in range(episode):  
   ...
   # q=q+(1/(i+1))*(returns[1,i]-q)#update q value
    ...
    elif epsilon>0.1 and i>=int(discover_percent*episode):#epsilon dont discount after 0.1
        epsilon*=epsilon_discount
    ...
    avg_step=[sum(a[i:i+10])/10 for i in range(0, len(a), 10)]# averge #least step / step count per 10 episode
...
file_name=directory+"\\"+v+"_rate_"+str(rate)+final_time+"_exam_number_"+str(n)+".jpg"
print(file_name)
